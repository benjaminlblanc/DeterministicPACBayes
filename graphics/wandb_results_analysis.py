...
def generate_latex_table(results):
    ...
    for i in range(len(results)):
        ...
        if results[i][1] == 'FO':
            ...
            FO_std = np.round(results[i][11] * 100, 1)
            FO_err = np.round(results[i][6] * 100, 1)
            FO_err_std = np.round(results[i][14] * 100, 1)
            Ben = np.round(results[i][2] * 100, 1)
            Ben_std = np.round(results[i][10] * 100, 1)
            Ben_err = np.round(results[i][7] * 100, 1)
            Ben_err_std = np.round(results[i][15] * 100, 1)
            Dis_KL = np.round(results[i][4] * 100, 1)
            Dis_KL_std = np.round(results[i][5] * 100, 1)
            Dis_KL_err = np.round(results[i][8] * 100, 1)
            Dis_KL_err_std = np.round(results[i][9] * 100, 1)
        elif results[i][1] == 'SO':
            SO = np.round(results[i][3] * 100, 1)
            SO_std = np.round(results[i][11] * 100, 1)
            SO_err = np.round(results[i][6] * 100, 1)
            SO_err_std = np.round(results[i][14] * 100, 1)
        elif results[i][1] == 'Bin':
            Bin = np.round(results[i][3] * 100, 1)
            Bin_std = np.round(results[i][11] * 100, 1)
            Bin_err = np.round(results[i][6] * 100, 1)
            Bin_err_std = np.round(results[i][14] * 100, 1)
        elif results[i][1] == 'Dis_Renyi':
            Dis_Renyi = np.round(results[i][4] * 100, 1)
            Dis_Renyi_std = np.round(results[i][5] * 100, 1)
            Dis_Renyi_err = np.round(results[i][8] * 100, 1)
...
num_results = ['ben_bound_with_finetune', 'deterministic_bound', 'deterministic_bound_sampled', 'deterministic_bound_sampled_std', 'test-error', 'test-error_finetune', 'test-error_sampled', 'test-error_sampled_std']  # Do not change that order
